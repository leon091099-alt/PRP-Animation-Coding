...
const SCENES: [string, number][] = [['Cities', 1.8], ['China', 1.4], ['Route', 1.8], ['Rhein', 1.3], ['Hold', 0.7]];
...
function tween(from: number, to: number, start: number, end: number, ease: Ease) {
  return (t: number) => (t <= start ? from : t >= end ? to : from + (to - from) * ease((t - start) / (end - start)));
}
const MOTION = {
  enter: (a: number, b: number, s: number, e: number) => tween(a, b, s, e, Easing.easeOutCubic),
  draw: (s: number, e: number) => tween(0, 1, s, e, Easing.easeInOutCubic),
  pop: (s: number, e: number) => tween(0, 1, s, e, Easing.easeOutBack),
};

// ---------- Welt (X Ost, Z Nord/fern, Y oben) ----------
// Warenweg: chinesische Städte → Perlfluss-Delta → Südchinesisches Meer → Seeroute → Nordsee → Rotterdam / Antwerpen → Rhein aufwärts → Köln
type P2 = [number, number];
const NODES: Record<string, P2> = {
  JC: [470, -430], P1: [640, -60], P2: [620, -280],
  fos: [380, 140], gzh: [510, 230], dgg: [720, 120], hui: [960, 210], sht: [1070, 340],
  jmn: [360, -130], zsn: [500, -200], zhh: [590, -350], szn: [800, -190], hkg: [880, -310],
  SEA: [-1150, 420], N1: [-900, 120],
  rot: [-1050, 260], ant: [-1110, -60], dui: [-800, 60], dus: [-760, -60], kol: [-630, -220],
};
(() => {
  const spread = (ids: string[], c: P2, kx: number, kz: number) => ids.forEach((id) => { const p = NODES[id]; NODES[id] = [c[0] + (p[0] - c[0]) * kx, c[1] + (p[1] - c[1]) * kz]; });
  spread(['JC', 'P1', 'P2', 'fos', 'gzh', 'dgg', 'hui', 'sht', 'jmn', 'zsn', 'zhh', 'szn', 'hkg'], [680, -20], 1.2, 1.15);
  spread(['N1', 'rot', 'ant', 'dui', 'dus', 'kol'], [-860, 20], 1.25, 1.2);
})();
interface City { id: string; en: string; zh: string; h: number; goal?: boolean; p: P2; order: number }
const CITIES: City[] = [
  { id: 'rot', en: 'Rotterdam', zh: '鹿特丹', h: 230 },
  { id: 'ant', en: 'Antwerp', zh: '安特卫普', h: 190 },
  { id: 'dui', en: 'Duisburg', zh: '杜伊斯堡', h: 270 },
  { id: 'dus', en: 'Düsseldorf', zh: '杜塞尔多夫', h: 100 },
...
function catmull(ctrl: P2[], per: number): P2[] {
  const out: P2[] = [];
  for (let i = 0; i < ctrl.length - 1; i++) {
    const p0 = ctrl[i - 1] || ctrl[i], p1 = ctrl[i], p2 = ctrl[i + 1], p3 = ctrl[i + 2] || p2;
    for (let j = 0; j < per; j++) {
      const t = j / per, t2 = t * t, t3 = t2 * t;
      out.push([0, 1].map((k) => 0.5 * (2 * p1[k] + (-p0[k] + p2[k]) * t + (2 * p0[k] - 5 * p1[k] + 4 * p2[k] - p3[k]) * t2 + (-p0[k] + 3 * p1[k] - 3 * p2[k] + p3[k]) * t3)) as P2);
    }
  }
...
function resample(pts: P2[], step: number): P2[] {
  const out: P2[] = [pts[0]]; let carry = 0;
...
function noise(x: number, seed: number) {
  const i = Math.floor(x), t = x - i, h = (k: number) => hash(i + k + seed * 101) * 2 - 1;
  const p0 = h(-1), p1 = h(0), p2 = h(1), p3 = h(2);
  return 0.5 * (2 * p1 + (-p0 + p2) * t + (2 * p0 - 5 * p1 + 4 * p2 - p3) * t * t + (-p0 + 3 * p1 - 3 * p2 + p3) * t * t * t);
}
function fbm(x: number, seed: number) { return noise(x, seed) * 0.7 + noise(x * 2.1, seed + 7) * 0.3; }
// unregelmäßige, aderartige Linie: zufällige Zwischenpunkte + rauschmodulierte Mäander
function vein(ctrl: P2[], seed: number, amp = 6, bow = 0.12): P2[] {
  if (ctrl.length === 2) {
    const [a, b] = ctrl, dx = b[0] - a[0], dz = b[1] - a[1], L = Math.hypot(dx, dz), nx = -dz / L, nz = dx / L;
    const k = 1 + Math.floor(hash(seed * 3) * 3), mids: P2[] = [];
    for (let j = 1; j <= k; j++) {
      const t = j / (k + 1) + (hash(seed * 11 + j) - 0.5) * 0.18;
      const o = (hash(seed * 17 + j) - 0.5) * 2 * L * bow * (0.6 + hash(seed * 23 + j));
  ...
  for (let i = 1; i < base.length; i++) { acc += Math.hypot(base[i][0] - base[i - 1][0], base[i][1] - base[i - 1][1]); lens.push(acc); }
  const L = acc;
  return base.map((p, i) => {
    ...
    return [p[0] - (tz / tl) * o, p[1] + (tx / tl) * o] as P2;
  });
}
// Kanten in Flussrichtung: [von, nach, Phase, Stufe, Breite von, Breite bis]
const RIVERS: [string, string, 'China' | 'Rhein', number, number, number][] = [
  ['sht', 'hui', 'China', 0, 1.1, 1.8], ['hui', 'dgg', 'China', 1, 1.8, 2.4], ['dgg', 'P1', 'China', 2, 2.4, 2.8],
  ['fos', 'gzh', 'China', 0, 1.2, 2.2], ['gzh', 'P1', 'China', 1, 2.2, 3.2],
  ['P1', 'P2', 'China', 3, 3.4, 4.2], ['szn', 'P2', 'China', 2, 1.2, 2.2], ['hkg', 'P2', 'China', 2, 1.2, 2.4],
  ['jmn', 'zsn', 'China', 1, 1.2, 2], ['zsn', 'zhh', 'China', 2, 2, 2.6], ['zhh', 'JC', 'China', 3, 2.6, 3.2],
  ['P2', 'JC', 'China', 4, 4.2, 5],
  ['SEA', 'rot', 'Rhein', 0, 5, 4.2], ['SEA', 'ant', 'Rhein', 0, 3.6, 3], ['rot', 'N1', 'Rhein', 1, 4.2, 3.8], ['ant', 'N1', 'Rhein', 1, 3, 2.6],
  ['N1', 'dui', 'Rhein', 2, 4.2, 3.6], ['dui', 'dus', 'Rhein', 3, 3.6, 3], ['dus', 'kol', 'Rhein', 4, 3, 2],
];
const LEVELS = { China: 5, Rhein: 5 };
const MAIN_CTRL: P2[] = [NODES.JC, [330, -440], [190, -350], [150, -250], [20, -170], [-20, -60], [-150, 10], [-210, 130], [-340, 170], [-420, 300], [-560, 350], [-640, 440], [-820, 410], [-960, 470], NODES.SEA];
interface Cap { pts: P2[]; u: number; w0: number; w1: number }
interface Edge { pts: P2[]; w0: number; w1: number; ph?: 'China' | 'Rhein'; lv?: number; caps?: Cap[] }
function withCaps(e: Edge, seed: number, n: number): Edge {
  const caps: Cap[] = [], count = n + Math.floor(hash(seed * 41) * (n + 2));
  for (let k = 0; k < count; k++) {
    ...
    const bpt: P2 = [p[0] + Math.cos(ang) * len, p[1] + Math.sin(ang) * len];
...
// Geometrie einmalig beim Laden (deterministisch)
...
// ---------- Kamera ----------
interface Cam { f: number; yaw: number; tilt: number; D: number; cx: number; cy: number }
function makeCam(T: number, drift: boolean): Cam {
...
function pathD(pts: P2[], cam: Cam) {
  return pts.map((p, i) => { const q = project(p[0], 0, p[1], cam); return (i ? 'L' : 'M') + q.x.toFixed(1) + ' ' + q.y.toFixed(1); }).join('');
}

// ---------- Bausteine ----------
function Ground({ T, cam }: { T: number; cam: Cam }) {
...
function ribbon(pts: P2[], w0: number, w1: number, prog: number, cam: Cam, K = 1.75) {
  if (prog <= 0) return null;
  const n = pts.length, cut = prog * (n - 1), m = Math.floor(cut), f = cut - m;
  const use = pts.slice(0, m + 1);
  if (m < n - 1 && f > 0) { const a = pts[m], b = pts[m + 1]; use.push([a[0] + (b[0] - a[0]) * f, a[1] + (b[1] - a[1]) * f]); }
  ...
  for (let i = 0; i < P.length; i++) {
    ...
    L.push([P[i].x - (ty / tl) * hw, P[i].y + (tx / tl) * hw]); R.push([P[i].x + (ty / tl) * hw, P[i].y - (tx / tl) * hw]);
  }
  const all = L.concat(R.reverse());
  return 'M' + all.map((p) => p[0].toFixed(1) + ' ' + p[1].toFixed(1)).join('L') + 'Z';
}
function Vein({ e, prog, cam, flow, flowOn, dash }: { e: Edge; prog: number; cam: Cam; flow: number; flowOn: number; dash: string }) {
  ...
  return (
    <g>
      ...
      {flowOn > 0 && prog >= 1 && <path d={pathD(e.pts, cam)} pathLength={1000} fill="none" stroke="#ffffff" strokeWidth={Math.max(1, (e.w0 + e.w1) * 0.28)}
        strokeLinecap="round" strokeDasharray={dash} strokeDashoffset={-flow} opacity={0.75 * flowOn} />}
    </g>
  );
}
function Veins({ T, cam, flow }: { T: number; cam: Cam; flow: number }) {
...
function Goal({ T, cam, amb }: { T: number; cam: Cam; amb: number }) {
  ...
  return <g opacity={on}>{rings.map((r, k) => r >= 0 && <ellipse key={k} cx={g.x} cy={g.y} rx={(14 + r * 110) * g.s} ry={(14 + r * 110) * g.s * 0.7}
    fill="none" stroke={C.deep} strokeWidth={1.4} opacity={1 - r} />)}</g>;
}
function cityTiming(c: City) { return CUES.Cities + 0.1 + (c.order % 5) * 0.12 + (c.order >= 5 ? Math.floor((c.order - 5) / 5) * 0.55 : 0.55 * hash(c.order)); }

// Kollisionsauflösung: Schilder werden in Bildschirmkoordinaten nach oben verschoben, bis sich keine mehr überlappen
type Lifts = Record<string, { lift: number; dx: number }>;
function layoutLabels(cam: Cam, chinese: boolean): Lifts {
  const PAD = 10;
  ...
  for (let it = 0; it < 40; it++) {
    ...
    for (let i = 0; i < L.length; i++) for (let j = i + 1; j < L.length; j++) {
      ...
      if (ov > 0) { up.lift += ov + 1; moved = true; }
    }
    if (!moved) break;
  }
  const M = 48;
...
function Pins({ T, cam, bobT, lifts }: { T: number; cam: Cam; bobT: number; lifts: Lifts }) {
...
function Signs({ T, cam, bobT, chinese, lifts, font }: { T: number; cam: Cam; bobT: number; chinese: boolean; lifts: Lifts; font: string }) {
  ...
  return <>{items.map(({ c, q }) => {
    ...
    const k = q.s * sc * 1.55;
    return (
      <div key={c.id} style={{ position: 'absolute', left: q.x, top: q.y, transform: `translate(-50%,-100%) scale(${k})`, transformOrigin: '50% 100%',
        opacity: Math.min(1, sc * 2), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, whiteSpace: 'nowrap', pointerEvents: 'none',
        filter: 'drop-shadow(0 22px 10px rgba(15,30,51,0.16)) drop-shadow(0 2px 1px rgba(15,30,51,0.12))' }}>
        <div style={{ fontFamily: font, fontWeight: 700, fontSize: c.goal ? 40 : 31, color: C.ink, lineHeight: 1, letterSpacing: '-0.005em',
          WebkitTextStroke: '5px #ffffff', paintOrder: 'stroke fill' } as any}>{c.en}</div>
        {chinese && <div style={{ fontFamily: '"PingFang SC","Noto Sans SC","Microsoft YaHei","Hiragino Sans GB",sans-serif', fontWeight: 600, fontSize: c.goal ? 21 : 17, color: C.deep, lineHeight: 1,
          letterSpacing: '0.35em', marginRight: '-0.35em', WebkitTextStroke: '4px #ffffff', paintOrder: 'stroke fill' } as any}>{c.zh}</div>}
...
// ---------- Einpassung ----------
// contain/cover: ganze Bühne. auto: Querformat = contain; Hochformat = Kamerafahrt
// (nah an China → Schwenk entlang der Route → Rheinland → Zoom auf das Gesamtbild zum Schluss).
const FOCUS = { china: [1370, 420], rhein: [420, 440], all: [960, 520] };
function fitView(w: number, h: number, fit: string, T: number) {
  if (!w || !h) return null;
  const kFull = Math.min(w / W, h / H);
  let k = fit === 'cover' ? Math.max(w / W, h / H) : kFull, fx = W / 2, fy = H / 2;
  if (fit === 'auto' && w / h < 1.1) {
    const kClose = Math.min(w / 1000, (h * 0.8) / 700);
    ...
    fx = cx + (FOCUS.all[0] - cx) * out; fy = cy + (FOCUS.all[1] - cy) * out;
  }
...
// ---------- Komponente ----------
interface Props {
  mode: 'scroll' | 'scrollParent' | 'autoplay'; scrollLength: number; scrollParentDepth: number; speed: number;
  previewTime: number; chinese: boolean; cameraDrift: boolean; flowSpeed: number; fit: 'auto' | 'contain' | 'cover';
  background: string; fontFamily: string;
}
const DEFAULTS: Props = {
  mode: 'scroll', scrollLength: 300, scrollParentDepth: 1, speed: 1, previewTime: 7, chinese: true, cameraDrift: true,
  flowSpeed: 1.4, fit: 'auto', background: '#ffffff', fontFamily: '"RPM Aglet Sans", Arial, sans-serif',
};
const clamp01 = (x: number) => (x < 0 ? 0 : x > 1 ? 1 : x);

function RiverScene(input: Partial<Props> & Record<string, any>) {
  const P: Props = { ...DEFAULTS };
  (Object.keys(DEFAULTS) as (keyof Props)[]).forEach((k) => { const v = input[k]; if (v !== undefined && v !== null && v !== '') (P as any)[k] = v; });
  ...
  // Boxgröße für die Einpassung der 1920×1080-Bühne
  useEffect(() => {
  ...
  // Editor: fester Frame
  useEffect(() => { if (isStatic) { setT(Math.min(TOTAL, Math.max(0, P.previewTime))); setAmb(0); } }, [isStatic, P.previewTime]);

  // Antrieb: Scroll / Eltern-Scroll / Autoplay, danach Idle
  useEffect(() => {
    if (isStatic) return;
    const root = rootRef.current;
    if (!root) return;
    if (reduced) { setT(TOTAL); setAmb(0); return; }
    let raf = 0, visible = false, cur = 0, last = -1, start = -1, endAt = -1;
    const target = (now: number) => {
      const p = propsRef.current;
      if (p.mode === 'autoplay') return start < 0 ? 0 : Math.min(TOTAL, ((now - start) / 1000) * p.speed);
    ...
    const tick = (now: number) => {
      ...
      if (!done) endAt = -1;
      setT(cur);
      setAmb(done ? (now - endAt) / 1000 : 0);
      if (visible && !document.hidden) raf = requestAnimationFrame(tick);
    };
    const wake = () => { if (!raf && visible) { last = -1; raf = requestAnimationFrame(tick); } };
  ...
  return (
    ...
    >
      <div ref={stageRef} style={scrollOwn ? { position: 'sticky', top: 0, height: '100vh', width: '100%', overflow: 'hidden' } : { position: 'absolute', inset: 0, overflow: 'hidden' }}>
        {view && (
          <div style={{ position: 'absolute', left: 0, top: 0, width: W, height: H, transform: `translate(${view.x}px,${view.y}px) scale(${view.k})`, transformOrigin: '0 0' }}>
